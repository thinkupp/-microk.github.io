<template>
  <vuetify-layout>
    ...
    <v-btn @click="$message.messageIcon('mdi-domain').success('自定义ICON')"
      >自定义ICON</v-btn
    >
    <v-btn @click="$message.messageIcon().success('隐藏预设的ICON')"
      >隐藏预设的ICON</v-btn
    >

    <v-subheader>过渡效果</v-subheader>
    <v-btn @click="$message.transition().success('随机过渡效果')"
      >随机过渡效果</v-btn
    >
    ...
    <v-btn
      ...
      >自定义背景色</v-btn
    >
    <v-btn @click="$message.offsetTop(Math.random() * 200).success('随机间距')"
      >随机间距</v-btn
    >

    <v-subheader>通过传参配置</v-subheader>
    <v-btn @click="$message({ color: '#12d6cd', message: '传参配置背景' })"
      >自定义背景色</v-btn
    >
    <v-btn
      @click="$message({ offsetTop: Math.random() * 200, message: '随机间距' })"
      >随机间距</v-btn
    >
...
export default class VuetifyMessage extends Vue {
  ...
  private handleVNodeCloaseButton() {
    this.$message
      .closeButtonContent(this.$createElement("div", "VNode Close"))
      .success("关闭按钮使用VNode");
  }
  ...
  private handleClose() {
    const bar = this.$message
      .top()
      .success("这个消息条将在1秒钟后通过close方法关闭");
